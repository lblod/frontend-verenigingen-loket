...
module('Unit | Utilities | tracked-data', function () {
  ...
  test('initial state', function (assert) {
    const person = new TrackedData<Person>({ givenName: 'John', familyName: 'Doe' });
  ...
  test('change tracking', function (assert) {
    ...
    assert.deepEqual(person.changedValues, ['givenName'], 'Tracks changed property');
    assert.ok(person.hasChanges, 'hasChanges is true after modification');

    person.data.familyName = 'Smith';
    assert.deepEqual(person.changedValues, ['givenName', 'familyName'], 'Tracks multiple changes');

    person.data.givenName = 'John';
    assert.deepEqual(person.changedValues, ['familyName'], 'Reverts to original values remove it from the changedValues list');

    person.data.familyName = 'Doe';
    assert.deepEqual(person.changedValues, [], 'Reverts to original values remove it from the changedValues list');
    assert.notOk(person.hasChanges, 'hasChanges is false when all properties are returned to their original value');
  });
  ...
  test('error management', function (assert) {
    ...
    assert.equal(person.errors.givenName, 'Required', 'Sets error for property');

    person.addError('givenName', 'Invalid');
    assert.equal(person.errors.givenName, 'Invalid', 'Overwrites existing error');
  ...
  test('acceptChanges resets baseline', function (assert) {
    ...
    assert.deepEqual(person.changedValues, [], 'changedValues is empty after acceptChanges');
    assert.notOk(person.isNew, 'after accepting changes the changes, the instance is no longer considered new either');
  });

  test('isNew flag respects options', function (assert) {
    const person1 = new TrackedData<Person>({ givenName: 'John', familyName: 'Doe' });
    assert.ok(person1.isNew, 'isNew defaults to true');

    const person2 = new TrackedData<Person>({ givenName: 'John', familyName: 'Doe' }, { isNew: false });
